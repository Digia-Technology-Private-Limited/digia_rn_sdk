...
export class VWContainer extends VirtualStatelessWidget<Props> {
    ...
    render(payload: RenderPayload): React.ReactNode {
        ...
        const color = payload.evalColor(self.props.get('color')) ?? 'transparent';
        const bgImage = self.props.get('bgImage');
        var borderRadiusProp: { [key: string]: any } = {};
        const borderRadius = To.borderRadius(self.props.get('borderRadius'));


        if (typeof borderRadius === 'object') Object.assign(borderRadiusProp, { ...borderRadius, overflow: 'hidden' });
        else if (typeof borderRadius === 'number') {
            borderRadiusProp.borderRadius = borderRadius;
            borderRadiusProp.overflow = 'hidden';
        }
        ...
        // Raw dimensions
        const widthRaw =
            !_isIntrinsic(rawWidth) && rawWidth != null
                ? _toDimensionValue(rawWidth)
                : undefined;

        const heightRaw =
            !_isIntrinsic(rawHeight) && rawHeight != null
                ? _toDimensionValue(rawHeight)
                : undefined;
